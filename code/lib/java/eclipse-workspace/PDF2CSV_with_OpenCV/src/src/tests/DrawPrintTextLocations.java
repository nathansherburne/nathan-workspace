...
/**
 * This is an example on how to get some x/y coordinates of text and to show
 * them in a rendered image.
 *
...
public class DrawPrintTextLocations extends PDFTextStripper {
	...
	private List<Rectangle2D> stringBboxes;
	 ...
	 */
	public DrawPrintTextLocations(PDDocument document) throws IOException {
		this.document = document;
		stringBboxes = new ArrayList<Rectangle2D>();
	}
     ...
     */
    public void process(int pageNumber) throws IOException {
		stripPage(pageNumber);          
    }

	public List<Rectangle2D> getStringBboxes() {
		return stringBboxes;
	}
	...
	protected void writeString(String string, List<TextPosition> textPositions) throws IOException {
		for (TextPosition text : textPositions) {
			...
			// note: text.getTextMatrix() is *not* the Text Matrix, it's the Text Rendering
			// Matrix
			AffineTransform at = text.getTextMatrix().createAffineTransform();

			// in red:
			// show rectangles with the "height" (not a real height, but used for text
			// extraction
			...
			stringBboxes.add(s.getBounds2D());

			// in blue:
			// show rectangle with the real vertical bounds, based on the font bounding box
			// y values
			// usually, the height is identical to what you see when marking text in Adobe
			// Reader
