...
LIB_DIR = paste0(ROOT_DIR, "code/lib/R/dengue/mexico/code/R/")
...
for(i in 1:length(CSV.filenames)) {
  ...
  # Mexico sometimes has commas instead of decimal points.
  
  # Incidence per 100,000 column changes location from year to year. So look for the column with the most (hopefully
  # the only column) decimal points. This is the rate column.
  matches = apply(df, 2,  function(column) length(grep('\\.', as.character(column))))
  rate.col.i = as.numeric(which(matches == max(matches)))
  browser()
