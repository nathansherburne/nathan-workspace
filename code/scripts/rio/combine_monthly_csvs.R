...
OUTPUT_FILENAME = "master_monthly.csv"
...
starting.col.names = c("Population", month.name, "Total.Cases")
...
### Read in CSVs
CSV.filenames = list.files(IN_DIR, pattern="*monthly.csv", full.names = TRUE)
...
for(i in 1:length(CSV.data.frames)) {
  ...
  rownames(df) = gsub("\\[[0-9]*\\]|\\*+", "", rownames(df))  # Some rownames have superscript note references (e.g. "rowname[1]"). Remove these superscripts.
  ...
  numeric = as.data.frame(apply(df,2,function(x)as.numeric(gsub("\\s+|\\.", '',x))))  # Remove spaces, decimal points
  ...
  fmt_df = melt(fmt_df, id.vars=c(YEAR_COL_NAME,PAREA_COL_NAME,ADMIN_COL_NAME,NEIGH_COL_NAME,POP_COL_NAME), variable.name=MONTH_COL_NAME, value.name=CASES_COL_NAME)
  # Sort
