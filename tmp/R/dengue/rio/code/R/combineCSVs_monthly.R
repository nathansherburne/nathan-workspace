...
CSV.DIR = paste0("~/Dropbox/LEPR03/nathan/Dengue/Rio/Data/Dengue_Classic/CSVs/FINAL_individual/monthly/")
...
for(i in 1:length(CSV.data.frames)) {
  ...
  tmp_jan_index = grep("Jan", colnames(fmt_df), ignore.case = TRUE)
  colnames(fmt_df)[seq(tmp_jan_index,tmp_jan_index+11)] = MONTHS
  ...
  fmt_df = melt(fmt_df, id.vars=c('Year','Planning_Area','Administrative_Region','Neighboorhood','Population'), variable.name="Month", value.name='Cases')
...
write.csv(CSV.df, paste0(CSVOutDir, "master_2011-2017_monthly.csv"), row.names = FALSE)
